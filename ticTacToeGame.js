...
//On the command line type "node TicTacToeGame.js" to start.
...
class TicTacToe {
  constructor(){
    // initiate variables
    this.ticTacToe = [1, 2, 3, 4, 5, 6, 7, 8, 9];
...
${this.displayItem(this.ticTacToe[6])} | ${this.displayItem(this.ticTacToe[7])} | ${this.displayItem(this.ticTacToe[8])}`;
  ...
  processGame(){
    ...
    if(this.moveRegister.length >= 5){
      ...
      // if no moves left
      if(this.ticTacToe.every(value => Number.isInteger(value) === false)){
        console.log('No more moves left. Game is a tie!');
        this.endGame();
      }
      checkSet.clear();
    }
  ...
  readMove(position){
    ...
    if(Number.isInteger(self.ticTacToe[(position - 1)]) === false){
  ...
  deleteLastMove(){
    ...
    if(this.moveRegister.length > 1){
      let pos = this.moveRegister[(this.moveRegister.length - 1)].position
      this.ticTacToe[pos] = ' '
      // unregister move 
